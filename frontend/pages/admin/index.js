    ...
    <div className="space-y-10 pb-20 max-w-[1600px] mx-auto px-4 lg:px-8">
      ...
      <section>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
          <div>
            <h1 className="text-4xl lg:text-5xl font-black text-teal-50 tracking-tighter flex items-center gap-4">
              Control <span className="text-emerald-500">Center</span>
              <span className="hidden sm:inline-block text-[10px] bg-emerald-600/20 text-emerald-500 px-3 py-1 rounded-full border border-emerald-600/20 font-black uppercase tracking-widest mb-2">v3.0</span>
            </h1>
            <p className="text-stone-400 mt-1 font-medium text-base lg:text-lg">Manage questions, categories, and student growth.</p>
          </div>
          <div className="flex gap-3">
             <Link href="/admin/questions/new?mode=standard" className="flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-emerald-50 rounded-2xl shadow-xl shadow-emerald-900/30 transition-all font-bold group text-sm lg:text-base">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          ...
          <StatCard icon={<Activity className="text-emerald-400" />} label="Total Admins" value={stats?.totalAdmins || 0} sub="System controllers" />
        </div>
      </section>
      ...
      <section className="animate-in fade-in slide-in-from-bottom-5 duration-700">
          <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl lg:text-3xl font-black text-teal-50 tracking-tighter flex items-center gap-3">
                  <Terminal className="text-emerald-500 w-7 h-7 lg:w-8 h-8" /> Practice <span className="text-emerald-500">Arena</span> Management
              </h2>
              <Link href="/admin/questions/new?mode=coding" className="text-[11px] lg:text-xs font-bold bg-white/5 border border-white/5 hover:border-emerald-500/30 px-4 py-2 rounded-xl text-emerald-500 transition-all uppercase tracking-widest font-black">
                  + Create Challenge
              </Link>
          </div>
                  ...
                  <div className="lg:col-span-4 space-y-6">
                      <div className="grid grid-cols-2 gap-3">
                          <div className="bg-stone-900/50 p-4 rounded-2xl border border-stone-800/50">
                              <p className="text-xl font-black text-teal-50">{codingChallenges.filter(c => c.difficulty === 'easy').length}</p>
                              <p className="text-[9px] uppercase font-black text-emerald-500 mt-1 tracking-widest">Easy</p>
                          </div>
                          <div className="bg-stone-900/50 p-4 rounded-2xl border border-stone-800/50">
                              <p className="text-xl font-black text-teal-50">{codingChallenges.filter(c => c.difficulty === 'medium').length}</p>
                              <p className="text-[9px] uppercase font-black text-emerald-500 mt-1 tracking-widest">Medium</p>
                          </div>
                          <div className="bg-stone-900/50 p-4 rounded-2xl border border-stone-800/50">
                              <p className="text-xl font-black text-teal-50">{codingChallenges.filter(c => c.difficulty === 'hard').length}</p>
                          ...
                          <div className="bg-stone-900/50 p-4 rounded-2xl border border-stone-800/50 flex items-center justify-center">
                              <Link href="/practice" className="text-stone-600 hover:text-emerald-500 transition-colors">
                          ...
                          {codingChallenges.map(challenge => (
                              <div key={challenge._id} className="bg-stone-950/40 border border-stone-800/40 p-4 rounded-2xl flex items-center justify-between hover:bg-white/[0.02] transition-all group/item px-6">
                                  <div className="flex items-center gap-5">
                                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black border ${challenge.difficulty === 'easy' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : challenge.difficulty === 'medium' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-50' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
                                          {challenge.difficulty.charAt(0).toUpperCase()}
                                      </div>
                                      <div>
                                          <div className="flex items-center gap-2">
                                              <span className="font-bold text-teal-50 text-sm truncate max-w-[200px] lgs:max-w-md">{challenge.questionText}</span>
                                  ...
                                  <div className="flex items-center gap-1.5">
                                      <Link href={`/admin/questions/${challenge._id}`} className="p-2.5 bg-stone-900 border border-stone-800 rounded-xl text-stone-500 hover:text-emerald-500 hover:border-emerald-500/30 transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-black text-teal-50 tracking-tighter flex items-center gap-3">
                    <Activity className="w-6 h-6 text-emerald-500" /> Recent <span className="text-emerald-500">Activity</span>
                </h2>
                <Link href="/admin/users" className="text-[10px] font-black text-emerald-500 hover:text-emerald-400 flex items-center gap-2 group uppercase tracking-widest">
                        ...
                        {recentUsers.map(user => (
                            <div key={user._id} className="p-5 flex items-center justify-between hover:bg-white/[0.01] transition-colors relative group/user">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-stone-950 border border-stone-800 flex items-center justify-center font-black text-emerald-500 group-hover/user:scale-110 transition-transform text-sm">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-0.5">
                                            ...
                                            <p className="text-[9px] uppercase font-bold text-emerald-700 tracking-wider">{user.points} XP</p>
                                        </div>
                                    </div>
                                </div>
          ...
          <div className="space-y-6">
              <h2 className="text-2xl font-black text-teal-50 tracking-tighter">Topic <span className="text-emerald-500">Armor</span></h2>
              <div className="bg-[#0c0a09] border border-stone-800 rounded-[2rem] p-6 lg:p-8 backdrop-blur-md space-y-6 h-fit shrink-0">
                  <div>
                    <div className="flex justify-between items-center mb-5">
                        ...
                        <span className="text-[8px] bg-stone-950 text-emerald-600 border border-stone-900 px-2 py-0.5 rounded-lg font-black uppercase tracking-widest">{categories.length} Modules</span>
                    </div>
                        ...
                        {stats?.distribution?.map(dist => (
                            <div key={dist.name} className="space-y-2 group/item">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-1 opacity-0 group-hover/item:opacity-100 transition-all">
                                            <button 
                                                ...
                                                className={`p-1 rounded bg-emerald-950 text-emerald-500 hover:text-emerald-400 transition-colors ${seeding === dist._id ? 'animate-pulse' : ''}`}
                                            >
                                ...
                                <div className="h-1.5 w-full bg-stone-950 rounded-full overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-emerald-600/60 to-emerald-500/20 rounded-full" style={{ width: `${Math.min(100, (dist.count / (mcqQuestions.length || 1)) * 100)}%` }}></div>
                                </div>
                            </div>
                        ))}
                    </div>
      ...
      <section className="pt-10 border-t border-stone-800/50">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
              <h3 className="text-2xl font-black text-teal-50 tracking-tighter shrink-0">Asset <span className="text-emerald-500">Repository</span></h3>
        ...
        <div className="space-y-6">
            <div className="relative group/search">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-stone-800 group-focus-within/search:text-emerald-500/50 transition-colors" />
                <input 
                    type="text" 
                    placeholder="Query the master bank..." 
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full bg-[#0d0b0a] border border-stone-800/80 rounded-[2rem] py-5 pl-16 pr-6 text-teal-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all font-medium placeholder:text-stone-800"
                />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredMCQs.slice(0, displayLimit).map(q => (
                    <div key={q._id} className="bg-[#12100f] border border-stone-800/50 p-5 rounded-3xl group hover:border-emerald-600/30 transition-all flex justify-between items-center shadow-lg relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-emerald-600/0 group-hover:bg-emerald-600/20 transition-all"></div>
                        <div className="min-w-0 px-2">
                            <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1.5 opacity-60">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
                            <Link href={`/admin/questions/${q._id}`} className="p-2 text-stone-600 hover:text-emerald-500 transition-colors">
            ...
            {filteredMCQs.length > displayLimit && (
                <div className="flex justify-center pt-8">
                    <button 
                        ...
                        className="px-8 py-3 bg-stone-900 border border-stone-800 rounded-2xl text-stone-400 hover:text-emerald-500 hover:border-emerald-500/30 transition-all font-black uppercase tracking-[0.2em] text-[10px]"
                    >
  ...
  return (
    <div className="bg-[#0f0d0c] border border-stone-800 p-6 lg:p-7 rounded-[2rem] transition-all hover:border-stone-700/50 group shadow-lg relative overflow-hidden">
      <div className="flex items-center gap-5 relative z-10">
        <div className="p-3.5 bg-stone-950 rounded-2xl border border-stone-800 group-hover:border-emerald-500/20 transition-all duration-500 shadow-inner group-hover:scale-105">
          {icon}
        </div>
        <div>
          <p className="text-stone-700 text-[9px] font-black uppercase tracking-widest leading-none mb-1.5">{label}</p>
          <div className="flex items-baseline gap-2">
             <p className="text-3xl font-black text-teal-50 leading-none">{value}</p>
...
function QuickActionCard({ icon, title, href, color }) {
    const colorClasses = {
        ...
        amber: "hover:border-emerald-500/40 hover:bg-emerald-500/[0.03] text-emerald-400"
    };
    return (
        <Link href={href} className={`p-4 bg-stone-950/80 border border-stone-800/50 rounded-xl transition-all group flex items-center gap-4 ${colorClasses[color] || 'text-stone-400 hover:border-emerald-500/30'}`}>
            ...
            <div className="flex flex-grow items-center justify-between">
                ...
                <PlusCircle className="w-3.5 h-3.5 text-stone-700 group-hover:text-emerald-500 group-hover:rotate-90 transition-all" />
            </div>
        </Link>
    );
}
...
function CategoryNavItem({ label, active, onClick, count }) {
    return (
        <button 
            type="button"
            onClick={onClick}
            className={`px-5 py-2 rounded-xl cursor-pointer transition-all text-[9.5px] font-black uppercase tracking-widest flex items-center gap-2 ${active ? 'bg-emerald-600/20 text-emerald-500 border border-emerald-600/10 shadow-lg' : 'text-stone-700 hover:text-stone-400 hover:bg-white/[0.02]'}`}
        >
            {label} 
            {count !== undefined && <span className={`px-1.5 py-0.5 rounded-md text-[8px] ${active ? 'bg-emerald-500 text-white' : 'bg-stone-900 text-stone-600'}`}>{count}</span>}
        </button>
    );
}
