      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col items-center mb-8">
            <div className="w-16 h-16 bg-stone-900 border border-stone-800 rounded-2xl flex items-center justify-center mb-4 shadow-2xl relative group">
                ...
                <ShieldCheck className="w-8 h-8 text-amber-500 relative z-10" />
            </div>
            <h1 className="text-3xl font-black text-orange-50 tracking-tighter">Access <span className="text-amber-500">Recovery</span></h1>
            <p className="text-stone-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2 opacity-60">System Protocol: Password Reset</p>
        </div>
        ...
        <div className="bg-[#0c0a09] border border-stone-800 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-stone-600 uppercase tracking-widest px-1">Registered Email Address</label>
                    <div className="relative group/input">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-700 group-focus-within/input:text-amber-500 transition-colors" />
                        <input
                            type="email"
                            placeholder="user@example.com"
                            className="w-full bg-stone-950 border border-stone-800 rounded-2xl py-4 pl-12 pr-4 text-orange-50 font-bold focus:outline-none focus:ring-2 focus:ring-amber-500/10 transition-all placeholder:text-stone-800"
                            value={email}
                ...
                <div className="pt-2">
                    ...
                    >
                        {loading ? 'Processing...' : (
                            <>
                                Send Recovery Link <Zap className="w-4 h-4 group-hover:scale-125 transition-transform" />
                            </>
                        )}
                    </button>
                </div>
